...
export default function PassportPage(){
  ...
  useEffect(()=>{(async()=>{
    const {data:{user}}=await supabase.auth.getUser(); if(!user){window.location.href='/login';return;} setUser(user);
    const [{data:p},{data:xp}] = await Promise.all([
      supabase.from('profiles').select('*').eq('id',user.id).maybeSingle(),
      supabase.from('experience_passports').select('*').eq('user_id',user.id).maybeSingle(),
    ]);
    if(p) setProfile({...profile,...p});
    if(xp) setPassport({...passport,...xp,commodities:(xp.commodities||[]).join(', '),lifecycle_areas:(xp.lifecycle_areas||[]).join(', '),delivery_roles:(xp.delivery_roles||[]).join(', '),operating_regions:(xp.operating_regions||[]).join(', '),...(xp.critical_experience||{})});
  })()},[]);

  async function save(e){e.preventDefault();setMessage('Saving…');
    ...
    setMessage(pErr||xErr ? (pErr||xErr).message : 'Experience Passport saved.');
  }
