...
export default function CompanyApply(){
  ...
  async function submit(e){
    ...
    if(!tier){setMessage('XL100 company status requires at least 100 commercial ETRM licenses.');return;}
    setSubmitting(true);
    ...
    const {error}=await supabase.rpc('submit_organization_application',{
      ...
      p_headquarters:form.headquarters||null,
      p_regions:form.regions.split(',').map(x=>x.trim()).filter(Boolean),
      p_claimed_license_count:Number(form.licenseCount),
      p_rationale:form.rationale||null,
  ...
  return <AppShell><main className="mx-auto max-w-5xl px-5 py-10"><div className="xl-kicker">Enterprise membership</div><h1 className="mt-3 text-4xl font-black">Apply for XL100 company status.</h1><p className="mt-3 max-w-3xl text-slate-400">Organizations with 100+ commercial ETRM licenses qualify to apply. License counts are self-declared on an honor system and reviewed by XferLogic; exact counts are not intended to be public.</p>
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_320px]"><form onSubmit={submit} className="xl-card grid gap-4 p-6 md:grid-cols-2"><label>Company name<input required disabled={submitted} value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label>Website<input disabled={submitted} value={form.website} onChange={e=>setForm({...form,website:e.target.value})}/></label><label>Industry / business<input disabled={submitted} placeholder="Integrated energy, utility, trading company…" value={form.industry} onChange={e=>setForm({...form,industry:e.target.value})}/></label><label>Headquarters<input disabled={submitted} value={form.headquarters} onChange={e=>setForm({...form,headquarters:e.target.value})}/></label><label>Regions<input disabled={submitted} placeholder="North America, Europe" value={form.regions} onChange={e=>setForm({...form,regions:e.target.value})}/></label><label>Approximate ETRM license count<input disabled={submitted} type="number" min="100" required value={form.licenseCount} onChange={e=>setForm({...form,licenseCount:e.target.value})}/></label><fieldset disabled={submitted} className="md:col-span-2"><legend className="mb-2 text-sm text-slate-400">Commercial ETRM products represented</legend><div className="grid gap-2 sm:grid-cols-2">{products.map(p=><label key={p.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 p-3"><input className="h-4 w-4" type="checkbox" checked={form.productIds.includes(p.id)} onChange={()=>toggle(p.id)}/><span>{p.vendor_name} · {p.product_name}</span></label>)}</div></fieldset><label className="md:col-span-2">Why should this organization be represented in XL100?<textarea disabled={submitted} rows="4" value={form.rationale} onChange={e=>setForm({...form,rationale:e.target.value})}/></label><div className="md:col-span-2 flex flex-wrap items-center gap-4"><button disabled={submitting||submitted} className="rounded-xl bg-cyan-400 px-5 py-3 font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-60">{submitted?'Application submitted':submitting?'Submitting…':'Submit company application'}</button>{message&&<span className="max-w-xl text-sm text-slate-400">{message}</span>}</div></form>
